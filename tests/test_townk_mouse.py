...
class TownkMouseTest(unittest.TestCase):
    ...
    def test_ordinary_key_makes_it_a_modifier(self) -> None:
        ...
        LIB.T_key(MB_GUI, True)
        self.assertEqual(self.mods(), MOD_LGUI, "Cmd should be held")
    ...
    # -- behaviour commit 2 will deliberately change ----------------------

    def test_scroll_does_not_convert_to_a_button_today(self) -> None:
        """Pins CURRENT behaviour, which the button-first rework will invert.

        pointing_device_task_kb() only inspects x/y, so scrolling (h/v) leaves
        a held key as a modifier by accident rather than by design. That
        happens to be what Cmd+scroll-to-zoom needs, so the reworked rules
        should make it deliberate -- and this test should then say so
        explicitly instead of describing a side effect.
        """
        LIB.T_key(MB_GUI, True)
        LIB.T_pointing(0, 0, 3, 3)

        self.assertEqual(self.mods(), MOD_LGUI, "scroll leaves it a modifier")
