...
#let keyLabel(layer, cluster, key) = {
  return [#layer.labels.at(cluster).at(key).replace(
    regex("%%([^[:blank:]%]+)"),
...
#let up-key(
  ...
  let hor_padding = if side == left { size.at(0) } else { -size.at(0) } * 0.12
  ...
  place(horizon + other_side,
    dx: -hor_padding,
    dy: -size.at(1) * 0.08,
    box(width: size.at(0) * 0.5)[#label]
  )
})
...
#let pad-key(
  ...
  place(horizon + other_side,
    dx: -hor_padding,
    dy: -size.at(1) * 0.05,
    box(width: size.at(0) * 0.35)[#label]
  )
})
...
#let thumb-cluster(
  ...
  // Up
  place(bottom + side,
    dx: side_aware_width * 0.1,
    dy: -cluster_height * 0.39,
    up-key(
      ...
      label: text(
        font: "Roboto", weight: "black", fill: white,
        fit-to-width(max-text-size: cluster_width * 0.1, keyLabel(layer, cluster, UP))
      ),
    )
  )

  // Pad
  let padLayerToggleColor = layerToggleColor(keymap, layer, if side == left {LEFT} else {RIGHT}, PAD)
  place(top + side,
    dy: cluster_height * 0.05,
    box({
      pad-key(
      ...
      if padLayerToggleColor != none {
        let padLayerToggleLabel = layerToggleLabel(layer, if side == left {LEFT} else {RIGHT}, PAD)
        place(horizon + side,
          dx: side_aware_width * 0.03,
          circle(
            ...
            #text(font: "Roboto", weight: "black",
              tracking: 1pt, size: cluster_height * 0.065, fill: white,
              padLayerToggleLabel
            )
          ]
        )
  ...
  // Nail
  let nailLayerToggleColor = layerToggleColor(keymap, layer, if side == left {LEFT} else {RIGHT}, NAIL)
  place(top + other_side,
    dy: cluster_height * 0.05,
    box({
      nail-key(
        size: (cluster_width * 0.395, cluster_height * 0.32),
      ...
      if nailLayerToggleColor != none {
        let nailLayerToggleLabel = layerToggleLabel(layer, if side == left {LEFT} else {RIGHT}, NAIL)
        place(horizon + other_side,
          dx: -side_aware_width * 0.03,
          circle(
            radius: cluster_height * 0.07,
  ...
  // Knuckle
  let knuckleLayerToggleColor = layerToggleColor(keymap, layer, if side == left {LEFT} else {RIGHT}, KNUCKLE)
  place(top + other_side,
    dy: cluster_height * 0.075 + cluster_height * 0.32,
    box({
      knuckle-key(
      ...
      if knuckleLayerToggleColor != none {
        let knuckleLayerToggleLabel = layerToggleLabel(layer, if side == left {LEFT} else {RIGHT}, KNUCKLE)
        place(horizon + other_side,
          dx: -side_aware_width * 0.03,
          circle(
            radius: cluster_height * 0.07,
            ...
            #set align(center + horizon)
            #text(font: "Roboto", weight: "black",
              size: cluster_height * 0.065, fill: white,
              knuckleLayerToggleLabel
            )
          ]
        )
      }
...
#let finger-cluster(
  ...
  // Center
  place(center + horizon, dy: -cluster_width * 0.145,
    circle(
      ...
      #set align(center + horizon)
      #text(font: "Roboto", weight: "regular", fill: white,
        fit-to-width(max-text-size: cluster_width * 0.16, keyLabel(layer, cluster, CENTER))
      )
    ]
  )

  // South
  place(bottom + center, dy: -cluster_width * 0.29,
    box({
      rect(
        ...
        #set align(center + horizon)
        #text(font: "Roboto", weight: "regular", fill: white,
          fit-to-width(max-text-size: cluster_width * 0.16, keyLabel(layer, cluster, SOUTH))
        )
      ]
      place(top,
        rect(
  ...
  place(bottom + center,
    box({
      ...
      place(center + horizon,
        box(inset: (top: cluster_width * 0.04, bottom: cluster_width * 0.02))[
        #text(font: "Roboto", weight: "regular", fill: white,
          fit-to-width(max-text-size: cluster_width * 0.14, keyLabel(layer, cluster, DSOUTH))
        )]
      )
    })
  )
  ...
  // East
  place(horizon + right, dy: -cluster_width * 0.145,
    box({
      rect(
        width: cluster_width * 0.33,
        height: cluster_width * 0.33,
        ...
        #set align(center + horizon)
        #text(font: "Roboto", weight: "regular", fill: white,
          fit-to-width(max-text-size: cluster_width * 0.16, keyLabel(layer, cluster, EAST))
        )
      ]
      place(left + top,
        rect(
  ...
  place(top + center,
    box({
      rect(
        ...
        #set align(center + horizon)
        #text(font: "Roboto", weight: "regular", fill: white,
          fit-to-width(max-text-size: cluster_width * 0.16, keyLabel(layer, cluster, NORTH))
        )
      ]
      place(bottom,
        rect(
  ...
  // West
  place(horizon + left, dy: -cluster_width * 0.145,
    box({
      rect(
        width: cluster_width * 0.33,
        height: cluster_width * 0.33,
        ...
        #set align(center + horizon)
        #text(font: "Roboto", weight: "regular", fill: white,
          fit-to-width(max-text-size: cluster_width * 0.16, keyLabel(layer, cluster, WEST))
        )
      ]
      place(right + top,
        rect(
...
#let boardSide(keymap, layer, side, size: array) = box(width: size.at(0), height: size.at(1), {
  ...
  if side == right {
    place(bottom + right, image("./images/svalboard-logo.png", width: 20%))
  } else {
...
#let layerKeymap(keymap, layer, size: array) = box({
  ...
  return [#boardSide(keymap, layer, left, size: boardSideSize)#h(sidesSpacing)#boardSide(keymap, layer, right, size: boardSideSize)]
})
